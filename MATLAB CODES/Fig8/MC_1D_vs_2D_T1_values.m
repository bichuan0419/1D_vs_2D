...
c1=0.3;c2=1-c1;
TE=8:8:512;
% TI = [0.01, 0.03, 50, 70, 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000, 1200, 1400, 1600, 1800, 2000, 3000, 4000, 6000, 8000,10000];
TI = 50:200:5000;
nruns=1000;
...
nbins_T2 = linspace(30,90,200);
nbins_c = linspace(0,1,100);
nbins_T1 = linspace(0,400,400);
...
parfor kk=1:nruns
noise = normrnd(0,STDnoise/sqrt(length(TI)),size(Z));
...
legend({['T_{2,1}', ', SD = ', num2str(std(TT21_1D),2)],['T_{2,2}', ', SD = ', num2str(std(TT22_1D),2)]},'FontSize',26, 'Location', 'NorthEast') 
...
legend({['c_1', ', SD = ', num2str(std(c1_1D),2)],['c_2', ', SD = ', num2str(std(c2_1D),2)]},'FontSize',26, 'Location', 'North') 
sgtitle( {['1D: T_{2,1} = ' num2str(T21), ', T_{2,2} = ' num2str(T22)]},'FontSize',40,'FontWeight','bold'); 
set(gcf,'position',[1845         159        1284         348])

% display something
disp(['SD(c1,c2,T21,T22) for 1D is [',num2str(std(c1_1D)), ', ',num2str(std(c2_1D)),', ', ...
    num2str(std(TT21_1D)),', ', num2str(std(TT22_1D)),']'])
...
noise = normrnd(0,STDnoise,size(Z));
ZN=Z+noise;
BN = lsqcurvefit(surfit, [T21+10*rand() T11+20*rand() T22+10*rand()...
    T12+20*rand() rand() rand()], XY, ZN, [],[],opt);
...
ax3 = nexttile;
histogram(TT21_2DA, nbins_T2)
hold on
histogram(TT22_2DA, nbins_T2)
xlabel('T_{2,1} or T_{2,2}','FontSize',40,'FontWeight','bold')
legend({['T_{2,1}', ', SD = ', num2str(std(TT21_2DA),2)],['T_{2,2}', ', SD = ', num2str(std(TT22_2DA),2)]},'FontSize',26, 'Location', 'NorthEast') 
% title( {['2D: T_{1,1}=' num2str(T11), ', T_{1,2}=' num2str(T12)]},'FontSize',30); 

ax4 = nexttile;

histogram(c1_2DA, nbins_c)
hold on
histogram(c2_2DA, nbins_c)
xlabel('c_1 or c_2','FontSize',40,'FontWeight','bold') 
legend({['c_1', ', SD = ', num2str(std(c1_2DA),2)],['c_2', ', SD = ', num2str(std(c2_2DA),2)]},'FontSize',26, 'Location', 'North') 
sgtitle( {['2D: T_{1,1} = ' num2str(T11), ', T_{1,2} = ' num2str(T12)]},'FontSize',40,'FontWeight','bold'); 
set(gcf,'position',[1845         159        1284         348])

% display something
disp(['SD(c1,c2,T21,T22) for 2D, with (T11,T12) = [',num2str(T11),', ',num2str(T12),'] is [', ...
    num2str(std(c1_2DA)),', ', num2str(std(c2_2DA)), ', ',num2str(std(TT21_2DA)),', ', num2str(std(TT22_2DA)), ']'])
...
ax5 = nexttile;
histogram(TT21_2DB, nbins_T2)
hold on
histogram(TT22_2DB, nbins_T2)
xlabel('T_{2,1} or T_{2,2}','FontSize',40,'FontWeight','bold')
legend({['T_{2,1}', ', SD = ', num2str(std(TT21_2DB),2)],['T_{2,2}', ', SD = ', num2str(std(TT22_2DB),2)]},'FontSize',26, 'Location', 'NorthEast') 
% title( {['2D: T_{1,1}=' num2str(T11), ', T_{1,2}=' num2str(T12)]},'FontSize',30); 

ax6 = nexttile;

histogram(c1_2DB, nbins_c)
hold on
histogram(c2_2DB, nbins_c)
xlabel('c_1 or c_2','FontSize',40,'FontWeight','bold') 
lgnd = legend({['c_1', ', SD = ', num2str(std(c1_2DB),2)],['c_2', ', SD = ', num2str(std(c2_2DB),2)]},'FontSize',26, 'Location', 'North') 
...
disp(['SD(c1,c2,T21,T22) for 2D, with (T11,T12) = [',num2str(T11),', ',num2str(T12),'] is [', ...
    num2str(std(c1_2DB)),', ', num2str(std(c2_2DB)), ', ',num2str(std(TT21_2DB)),', ', num2str(std(TT22_2DB)), ']'])
...
ax7 = nexttile;
histogram(TT21_2DC, nbins_T2)
hold on
histogram(TT22_2DC, nbins_T2)
xlabel('T_{2,1} or T_{2,2}','FontSize',40,'FontWeight','bold')
legend({['T_{2,1}', ', SD = ', num2str(std(TT21_2DC),2)],['T_{2,2}', ', SD = ', num2str(std(TT22_2DC),2)]},'FontSize',26, 'Location', 'NorthEast') 
% title( {['2D: T_{1,1}=' num2str(T11), ', T_{1,2}=' num2str(T12)]},'FontSize',30); 

ax8 = nexttile;

histogram(c1_2DC, nbins_c)
hold on
histogram(c2_2DC, nbins_c)
xlabel('c_1 or c_2','FontSize',40,'FontWeight','bold') 
lgnd = legend({['c_1', ', SD = ', num2str(std(c1_2DC),2)],['c_2', ', SD = ', num2str(std(c2_2DC),2)]},'FontSize',26, 'Location', 'North') 
lgnd.BoxFace.ColorType='truecoloralpha';
lgnd.BoxFace.ColorData=uint8(255*[1 1 1 0.75]');
sgtitle( {['2D: T_{1,1} = ' num2str(T11), ', T_{1,2} = ' num2str(T12)]},'FontSize',40,'FontWeight','bold'); 
set(gcf,'position',[1845         159        1284         348])
...
disp(['SD(c1,c2,T21,T22) for 2D, with (T11,T12) = [',num2str(T11),', ',num2str(T12),'] is [', ...
    num2str(std(c1_2DC)),', ', num2str(std(c2_2DC)), ', ',num2str(std(TT21_2DC)),', ', num2str(std(TT22_2DC)), ']'])

linkaxes([ax1,ax3,ax5,ax7],'xy')
linkaxes([ax2,ax4,ax6,ax8],'xy')
% 
% set(gcf,'position',[187         353        1106         235])

%% save figures
saveas(h1,'MC_1D_vs_2D_A','epsc')
saveas(h2,'MC_1D_vs_2D_B','epsc')
saveas(h3,'MC_1D_vs_2D_C','epsc')
saveas(h4,'MC_1D_vs_2D_D','epsc')